...
class statisticsController extends Controller
{
    ...
    */ 
    public function indexME(Request $request)
    {
        ...
        if ($myGroup > 0){
            ...
            $balance            = $this->getBalanceME($myGroup, $fechaDesde , $fechaHasta ,$myCoin ,$myWallet);

             $balanceBefore      = $this->getBalanceBeforeME($myGroup,$fechaDesde, $fechaHasta, $myCoin, $myWallet);
            //  dd($balance);
            // dd($balanceBefore);
            // dd('myGroup -> ' . $myGroup . 'Fecha desde Before -> ' . $fechaDesde . ' - Fecha hasta before -> ' . $fechaHasta . '  myCoin -> ' . $myCoin);
        }
        else
        {
            if ($myWallet > 0){
        ...
        // dd($balance);
        // dd($balanceBefore);
        // dd('fechaDesde -> ' . $fechaDesde . ' - fechaHasta ->' . $fechaHasta);
        $myUser         = 0;
        $myUserDesde    = 0;
        $myUserHasta    = 9999;
        ...
        */
        ...
        //
        // Filtra bsqueda segun lo que el role tiene asignado para ver en cajas y grupos
        //
         $busquedaGroupFilter      = "";
         $busquedaWalletFilter     = "";
        // if ($myGroup != 0){
        ...
        // dd($myFechadesdeInvertida);
        // \Log::info('leam ----> ' .  json_encode($Transacciones, JSON_PRETTY_PRINT));
        return view('estadisticas.indexME', $parametros);
    ...
    *
    */
    public function index_allExterno(Request $request)
    {
        // \Log::info('Inicio index_all2');

        $Group_roles = $this->getGroupRole(auth()->id());
         \Log::info('leam - el user id es index_allExterno -> ' . print_r($Group_roles,true));
         \Log::info('leam - el user id es index_allExterno - count(wallets) -> ' . print_r(count($Group_roles->wallets),true));
         \Log::info('leam - el user id es index_allExterno - count(groups) -> ' . print_r(count($Group_roles->groups),true));

        if (count($Group_roles->wallets) > 0){
            $request->wallet = $Group_roles->wallets[0];
        } 
        ...
        \Log::info('leam - index_allExterno - $request->wallet - > ' . $request->wallet);
        \Log::info('leam - index_allExterno - $request->grupo - > '  . $request->grupo);
        ...
        */
        ...
        // dd($myFechadesdeInvertida);
        // \Log::info('leam ----> ' .  json_encode($Transacciones, JSON_PRETTY_PRINT));
        return view('estadisticasExterno.index', $parametros);
    ...
    */
    function getWalletSummaryME(Request $request) {
        ...
        // dd($fechaDesde . " - " . $fechaHasta);
        // dd('getWalletSummaryME - con ->  ' . $myCoin);
...
/* 
    ...
    *
    */
    function getBalanceME($grupo = 0, $myFechaDesde = "2001-01-01", $myFechaHasta = "9999-12-31", $typeCoin = null, $wallet = null){

        $busquedaFiltroWallet = "";
        if ($wallet){
            $busquedaFiltroWallet = "and wallet_id = " . $wallet;
            $walletDesde = $grupo;
            $walletHasta = $grupo;
    ...
    *
    */
    function getBalanceBeforeME($myGroup = 0, $myFechaDesde = "2001-01-01", $myFechaHasta = "9999-12-31", $myCoin = 1, $wallet = null){
        ...
        */
        ...
        if ($myFechaDesde === "2001-01-01"){
            $theTransaction = new stdClass();
            $theTransaction->IdGrupo = $myGroup;
            $theTransaction->NombreGrupo = '';
            $theTransaction->Cant = 0;
            $theTransaction->Creditos = 0;
            $theTransaction->Debitos = 0;
        ...
        if ($myGroup > 0){
            ...
            \Log::info('paso en before - calcula antes myCoin       ->' . $myCoin);
        */
            // dd('myFechaDesdeBefore -> ' . $myFechaDesdeBefore . ' - myFechaHastaBefore -> ' . $myFechaHastaBefore);
            $balance3           = $this->getBalanceME($myGroup, $myFechaDesdeBefore, $myFechaHastaBefore, $myCoin, $wallet);
             // dd($balance3);
            // if ($myGroup == 17)  dd($balance3);

            // dd('las fechas - ' . $balance3->Total . ' grupo ' . $myGroup . 'fecha desde -> ' . $myFechaDesdeBefore . ' fecha hasta -> ' . $myFechaHastaBefore . ' - coin -> ' . $myCoin);
    ...
    */
    function getBalanceWalletBeforeME($myWallet = 0, $myFechaDesde = "2001-01-01", $myFechaHasta = "9999-12-31", $myCoin = 1){
        ...
        $theTransaction = new stdClass();
        $theTransaction->IdWallet = $myWallet;
        ...
        */
        ...
        if ($myWallet > 0){
            ...
            // dd($balance3);
            // foreach($balance3 as $item){
            //     $balanceDetail  += $item->Total;
            // }
    ...
    */
    function getBalanceWalletME($wallet = null, $fechaDesde = "2001-01-01", $fechaHasta = "9999-12-31", $myCoin = null){
        ...
        */
        ...
        //         $Item->TotalDolar = $Item->TotalDolar;
        // }
        // dd($Transacciones);
    ...
    *
    */
    function ResumenEntradaWallet($theWallet = null, $myFechaDesde2 = "", $myFechaHasta2 = ""){
        // \Log::info('leam - statisticsController - commissionsProfit - el wallet es ->' . $request->wallet);
        // $request->wallet        = 89;   // abu mahmud
        // $request->wallet        = 93;   // caja usdt
        // $request->wallet        = 139;  // caja principal usdt
        ...
        $myTransaction     = 13; // 13 cobros usdt
    ...
    *
    *
    *       ResumenEntradaWalletComision
    *       recibe grupos separados por comas
    *
    */
    function ResumenEntradaWalletComision($theWallet, $groups = "", $fechaDesde = "", $fechaHasta = ""){
...
public function getEnterprise(Request $request){
    ...
    $enterprise = Enterprise::orderBy('name','ASC')->pluck('name', 'id')->toArray();
...
public function cajaMayorCuadroMovimientos(request $request)
{
   ...
   // dd($myFechaDesde);
    ...
    if ($myWallet != 0){
        ...
        $myJson2         = file_get_contents("filtros\myCajaMayorWalletsFiltros");
        $myJsonData2     = json_decode($myJson2,true);
        //  dd($myJsonData2);

        // dd($cajaMayorWallets);
        
        $RecargasWallet = collect([]);
        $transaccionesGrupoComision = collect([]);
        $transaccionesGrupoSalida = collect([]);
        $transaccionesGrupoSalida2 = collect([]);
        $transaccionesGrupoSalida3 = collect ([]);
        $transaccionesWalletsSalida3 = collect([]);

        foreach($cajaMayorWallets as $cajaMayorItem){

            $Recargas             = $this->ResumenEntradaWallet($cajaMayorItem, $myFechaDesde, $myFechaHasta);

            $indFiltro = false;
            $transaccionesGrupoComisionTemp = collect([]);
            $transaccionesGrupoSalidaTemp = collect([]);
            $transaccionesGrupoSalida2Temp = collect([]);
            $transaccionesGrupoSalida3Temp = collect ([]);
            ...
            foreach($myJsonData2 as $item){
                ...
                if ($item['wallet'] == $cajaMayorItem){
                    ...
                    $transaccionesGrupoComisionTemp = $this->ResumenEntradaWalletComision($cajaMayorItem, $myGroups, $myFechaDesde, $myFechaHasta);

                    $myGroups                   = $item['groupsSalida1'];
                    $transaccionesGrupoSalidaTemp   = $this->ResumenSalidaWalletGroup($cajaMayorItem, $myGroups);

                    $myGroups                   = $item['groupsSalida2'];
                    $transaccionesGrupoSalida2Temp   = $this->ResumenSalidaWalletGroup($cajaMayorItem, $myGroups);        
            
                    $myGroups                   = $myJsonData['groupsSalida3'];
                    $transaccionesGrupoSalida3Temp   = $this->ResumenSalidaWalletGroup($cajaMayorItem, $myGroups);

                    $myGroups = $myJsonData['walletsSalida3'];
                    $transaccionesWalletsSalida3Temp   = $this->ResumenSalidaWalletGroup($cajaMayorItem, $myGroups);

                    $indFiltro = true;

                    break;
                }
                // \Log::info('jsonData2 -> wallet -> ' . print_r($item['wallet'],true));
                // \Log::info('jsonData2 -> wallet -> ' . print_r($var,true));
                
            }
            if ($indFiltro == false){
                $transaccionesGrupoComisionTemp = $this->ResumenEntradaWalletComision($cajaMayorItem, null, $myFechaDesde, $myFechaHasta);
                $transaccionesGrupoSalidaTemp   = $this->ResumenSalidaWalletGroup($cajaMayorItem, null);
                $transaccionesWalletsSalida3Temp   = $this->ResumenSalidaWalletGroup($cajaMayorItem, null);
            }
            
            // \Log::info( "leam *** statisticsController -> los groups  -> del wallet $cajaMayorItem -> " . print_r($myGroups,true));
            
            \Log::info( "leam *** statisticsController -> recargas  -> del wallet $cajaMayorItem -> " . print_r($Recargas,true));

            $RecargasWallet = $RecargasWallet->concat($Recargas);
            $transaccionesGrupoComision = $transaccionesGrupoComision->concat($transaccionesGrupoComisionTemp);
            $transaccionesGrupoSalida = $transaccionesGrupoSalida->concat($transaccionesGrupoSalidaTemp);
            $transaccionesGrupoSalida2 = $transaccionesGrupoSalida2->concat($transaccionesGrupoSalida2Temp);
            $transaccionesGrupoSalida3 = $transaccionesGrupoSalida3->concat($transaccionesGrupoSalida3Temp);
            $transaccionesWalletsSalida3 = $transaccionesWalletsSalida3->concat($transaccionesWalletsSalida3Temp);
    ...
    return view('cajaMayor.cajaMayorCuadroMovimientos', $parametros);
    ...
    *
    *
    * Resumen SalidaWalletGroup
    * Pagos que ha realizado un wallet de la caja mayor
    * comisiones 
    */
    function ResumenSalidaWalletGroup($theWallet = "", $groups ="", $fechaDesde = "", $fechaHasta=""){
