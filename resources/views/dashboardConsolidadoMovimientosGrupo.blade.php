...
<!--
    ...
    resumen de comisoipnes grupo USDT 
    desde tabla commissiones_usdt
...
<div class="container justify-content-center" style="display: contents;">
    ...
    <div class="row col-12 col-md-12 justify-content-center text-center align-items-center" style="min-height: 5rem !important">
        <h4>Consolidado de Movimientos por Grupo</h4>
    </div>
        ...
        $('#myTable').DataTable({
            ...
            'buttons':[
                ...
                },
                {
                    ...
                    title: 'MTF | Cobros del Proveedor',
                    ...
                    <div class="col-12 col-md-12">
                        <table class="table thead-light" style="background-color: white;">
                            ...
                            @php
                                ...
                                $cant                           = 0;
                                $totalAmount                    = 0;
                                $totalAmountComision            = 0;
                                $totalAmountComisionBase        = 0;
                                $totalAmountComisionProfit      = 0;
                            ...
                            @if(count($Transacciones))
                                ...
                                @foreach($Transacciones as $Transaccion)
                                    @php
                                        ...
                                        $myContinue = 0;
                                        if ($Transaccion->TypeTransactionGroup == '1'){
                                    ...
                                    @if($myContinue == 0)
                                        ...
                                        @continue;
                                    @endif
                                    <tr class="myTr" onClick="theRoute2({{0}}, {{$Transaccion->GroupId}}, {{0}}, {{$Transaccion->TypeTransactionId}})">

                                        @php

                                            $totalAmount                +=  $Transaccion->Amount;                                    
                                            $totalAmountComision        +=  $Transaccion->AmountCommission;
                                            $totalAmountComisionBase    +=  $Transaccion->AmountCommissionBase;
                                            $totalAmountComisionProfit  +=  $Transaccion->AmountCommissionProfit;

                                            $cant                       += $Transaccion->TransactionCount;

                                        @endphp



                                        <td>{{ $Transaccion->TypeTransactionName}}</td>
                                        <td>{{ number_format($Transaccion->TransactionCount) }}</td>
                                        <td>{{ number_format($Transaccion->Amount ,2) }}</td>
                                        <td>{{ number_format($Transaccion->AmountCommission,2) }}</td>
                                        <td>{{ number_format($Transaccion->AmountCommissionBase,2) }}</td>
                                        <td>{{ number_format($Transaccion->AmountCommissionProfit,2) }}</td>
                                ...
                                <tr style="background-color: black; color:white;">
                                    ...
                                    <td                     >{{ number_format($totalAmountComision, 2) }}</td>
                                    <td                     >{{ number_format($totalAmountComisionBase, 2) }}</td>
                                    <td                     >{{ number_format($totalAmountComisionProfit,2) }}</td>
                                </tr>
                            @else
                                <tr class="myTr"">
                                    <td colspan="6">
                            ...
                            @if(count($Transacciones))
                                ...
                                @foreach($Transacciones as $Transaccion)
                                    @php
                                        ...
                                        $myContinue = 0;
                                        if ($Transaccion->TypeTransactionGroup == '2'){
